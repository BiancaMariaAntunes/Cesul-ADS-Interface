...
function TerceiraTela(){
    ...
    const {valor, subtrair} = useContext(CalculadoraContext);
 ...
 return(
    ...
    <Header/>
        ...
        <section className={objeto.section2}>
            ...
            <p className={objeto.p}>Informe um número para subtrair</p>
            <input className={objeto.input} type="number" onChange={(event) => setValorNovo(event.target.value)} />
            <Link to={"/multiplicar"}>
                <button className={objeto.button} onClick={() => subtrair(valorNovo)}>Próximo</button>
            </Link>
